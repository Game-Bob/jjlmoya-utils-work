...
export const content: ToolLocaleContent<ResignationLetterGeneratorUI> = {
  ...
  { type: 'paragraph', html: "使用前请结合计算器显示的假设一起解读结果。" },
  { type: 'paragraph', html: "请检查输入值、单位、四舍五入方式、日期和司法辖区，因为每项都可能改变估算。" },
  { type: 'paragraph', html: "发送前请仔细检查信件。确认收件人、最后工作日、理由、签名和提交方式符合合同及适用规则，并保存信件副本和送达记录，以便完整留档。文字应简洁、礼貌，只写必要信息，并在发送前确认日期和联系方式无误。再次核对生效日期，并保存所有相关沟通记录。如对通知期限有疑问，请查看合同或寻求合适的专业建议。签署前也请确认联系方式正确，并再次核对日期。发送前再做一次文字检查，确保信息清楚完整。提交后继续保存相关记录，并确认对方已经收到文件。请保存邮件或回执，以便日后查询和证明提交时间。保留完整资料。确认文件已提交。" },],
};
