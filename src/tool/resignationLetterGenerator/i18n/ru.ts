...
export const content: ToolLocaleContent<ResignationLetterGeneratorUI> = {
  ...
  { type: 'paragraph', html: "Перед использованием интерпретируйте результат вместе с допущениями, указанными калькулятором." },
  { type: 'paragraph', html: "Проверьте ввод, единицы, округление, дату и юрисдикцию: каждый параметр может изменить оценку." },
  { type: 'paragraph', html: "Перед отправкой внимательно перечитайте письмо. Проверьте получателя, последний рабочий день, причину, подпись и способ передачи с учётом договора и действующих правил. Сохраните копию документа и подтверждение получения, чтобы весь процесс был зафиксирован. Пишите кратко, вежливо и указывайте только необходимые сведения. Ещё раз проверьте дату прекращения договора и сохраните всю связанную переписку. Если срок уведомления вызывает сомнения, проверьте договор или обратитесь за подходящей консультацией. Перед подписью проверьте контактные данные." },],
};
